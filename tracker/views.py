...
@login_required
def customer_register_sales_redirect(request: HttpRequest):
    """Handle customer registration with redirect to order creation for sales."""
    if request.method == 'POST':
        form = CustomerStep1Form(request.POST)
        if form.is_valid():
            # Save customer data in session
            request.session['sales_customer_data'] = form.cleaned_data
            # Redirect to order creation with customer data
            return redirect('{}?intent=sales'.format(reverse('tracker:order_create')))
    else:
        form = CustomerStep1Form()
    
    return render(request, 'tracker/customer_register.html', {
        'form': form,
        'step': 1,
        'intent': 'sales',
        'is_sales_redirect': True
    })
...
def customer_register(request: HttpRequest):
    ...
    context = {}
    # Initialize order_initial and sel_services if needed
    order_initial = {}
    sel_services = []
    
    # Check if this is a sales redirect
    if request.GET.get('intent') == 'sales' and request.method == 'GET':
        return redirect('tracker:customer_register_sales_redirect')
    ...
    if request.method == "POST":
        if step == 1:
            ...
            if form.is_valid():
                ...
                request.session["intent"] = form.cleaned_data.get("intent")
                return redirect(f"{reverse('tracker:customer_register')}?step=3")
        elif step == 3:
            ...
            # Manually set the order type based on intent if not set
            intent = request.session.get("step2", {}).get("intent")  
            if intent == 'sales' and 'type' not in request.POST:
                order_form.data = order_form.data.copy()
                order_form.data['type'] = 'sales'
            
            # Check if all forms are valid
            form_valid = form.is_valid()
            vehicle_form_valid = vehicle_form.is_valid() if any(vehicle_form.data.values()) else True
            order_form_valid = order_form.is_valid()
            
            # Add forms to context for re-display if validation fails
            context.update({
                'form': form,
                'vehicle_form': vehicle_form,
                'order_form': order_form,
                'form_valid': form_valid,
                'vehicle_form_valid': vehicle_form_valid,
                'order_form_valid': order_form_valid,
            })
    
    # Initialize form with a default empty form for step 4
    form = None
    
    # Handle GET requests or re-display forms with validation errors
    if step == 1:
        initial = request.session.get('step1', {})
        form = CustomerStep1Form(initial=initial)
    elif step == 2:
        form = CustomerStep2Form(initial={'intent': request.session.get('intent')})
    elif step == 3:
        form = CustomerStep3Form(initial=request.session.get('step3', {}))
        context["order_form"] = OrderForm(initial=order_initial)
    
    # Add common context variables
    context.update({
        'step': step,
        'form': form if form is not None else CustomerStep4Form(initial=request.session.get('step4', {})),
        'intent': request.session.get('intent'),
    })
...
def order_start(request: HttpRequest):
    ...
    if request.method == 'GET':
        # Check for sales customer data in session first
        sales_customer_data = request.session.pop('sales_customer_data', None)
        if sales_customer_data:
            # Create the customer from session data
            try:
                customer = Customer.objects.create(
                    full_name=sales_customer_data.get('full_name', ''),
                    phone=sales_customer_data.get('phone', ''),
                    email=sales_customer_data.get('email', ''),
                    address=sales_customer_data.get('address', ''),
                    notes=sales_customer_data.get('notes', ''),
                    customer_type=sales_customer_data.get('customer_type'),
                    organization_name=sales_customer_data.get('organization_name', ''),
                    tax_number=sales_customer_data.get('tax_number', ''),
                    personal_subtype=sales_customer_data.get('personal_subtype', '')
                )
                # Pre-fill the order form with sales type
                form = OrderForm(initial={'type': 'sales'})
                form.fields['vehicle'].queryset = customer.vehicles.all()
                return render(request, "tracker/order_create.html", {
                    "customer": customer, 
                    "form": form,
                    "from_sales_redirect": True
                })
            except Exception as e:
                messages.error(request, f"Error creating customer: {str(e)}")
                return redirect('tracker:customer_register_sales_redirect')
        
        # Handle regular customer ID in URL
...
def api_inventory_brands(request: HttpRequest):
    ...
    data = cache.get(cache_key)
    if not data:
        # Get all inventory items for this product name
        items = InventoryItem.objects.filter(name=name)
        
        # Group by brand
        brand_data = {}
        for item in items:
            brand_name = (item.brand or "").strip() or "Unbranded"
            ...
            brand_data[brand_name]["quantity"] += (item.quantity or 0)
            if item.price is not None:
                if brand_data[brand_name]["price"] is None:
                    brand_data[brand_name]["price"] = item.price
                else:
                    brand_data[brand_name]["price"] = min(brand_data[brand_name]["price"], item.price)
        
        # Convert to list and sort by brand name
        brands = sorted(brand_data.values(), key=lambda x: x["brand"])
        ...
        } for b in brands if b["quantity"] > 0]  # Only include in-stock items
        
        data = {"brands": formatted_brands}
        cache.set(cache_key, data, 120)  # Cache for 2 minutes
...
def api_recent_orders(request: HttpRequest):
    ...
    orders = Order.objects.select_related('customer', 'vehicle').order_by('-created_at')[:10]
    data = []
    for order in orders:
        data.append({
            'id': order.id,
            ...
            'vehicle_plate': order.vehicle.plate_number if order.vehicle else None,
        })
    return JsonResponse({'results': data})
...
def api_inventory_items(request: HttpRequest):
    ...
    items = InventoryItem.objects.values('name').distinct().order_by('name')
...
@login_required 
def api_inventory_brands(request: HttpRequest):
    """API endpoint for inventory brands by item name"""
    name = request.GET.get('name', '').strip()
    if not name:
        return JsonResponse({'brands': []})
    
    brands = InventoryItem.objects.filter(
        name__iexact=name
    ).values('brand', 'quantity', 'price').order_by('brand')
    
    # Convert to list and handle None brands
    brands_list = []
    for item in brands:
        brand_name = item['brand'] or 'Unbranded'
        brands_list.append({
            'brand': brand_name,
...
def api_inventory_stock(request: HttpRequest):
    ...
    try:
        # Handle "Unbranded" case
        if brand.lower() == 'unbranded':
            item = InventoryItem.objects.get(name__iexact=name, brand__isnull=True)
        else:
