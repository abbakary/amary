...
class OrderForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        # If type is not set, try to get it from the POST data
        if not t and hasattr(self, 'data') and 'type' in self.data:
            t = self.data.get('type')
            cleaned['type'] = t
        
        if t == "sales":
            # Check required fields for sales orders
            required_fields = {
            ...
            # Check required fields for service orders
            if not cleaned.get("description"):
                self.add_error("description", "Please describe the problem or service needed")
            if not cleaned.get("estimated_duration"):
                self.add_error("estimated_duration", "Please provide an estimated duration")
                
            # Append selected services to description
            services = cleaned.get("service_selection") or []
            if services:
                desc = cleaned.get("description") or ""
                services_list = ", ".join(dict(self.SERVICE_OPTIONS).get(s, s) for s in services)
                cleaned["description"] = f"{desc}\n\nSelected services: {services_list}".strip()
                
        elif t == "consultation":
            # Check required fields for consultations
            if not cleaned.get("inquiry_type"):
                self.add_error("inquiry_type", "Please select an inquiry type")
            if not cleaned.get("questions"):
                self.add_error("questions", "Please provide your questions")
        else:
            # If we get here, the type might be missing or invalid
            if not t:
                self.add_error(None, "Order type is required")
            else:
                self.add_error("type", f"Invalid order type: {t}")
